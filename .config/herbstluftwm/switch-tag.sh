...
target_tag=$(for tag_status in "${tags[@]}" ; do
                 ...
                 tag="${tag_status:1}"
                 echo $tag
             done | rofi -show-icons -terminal urxvtc -dmenu -p "hlwm: switch tag")

if [ -z "$target_tag" ]; then
    exit 0
fi

if ! herbstclient use $target_tag; then
    resp=$(echo $'y\nn' | rofi -show-icons -terminal urxvtc -dmenu -p "hlwm: create tag $target_tag?")
    if [[ z"$resp" == zy ]]; then
        herbstclient add $target_tag
    fi
fi
